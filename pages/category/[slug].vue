    ...
    <div>
        ...
        <div v-else>
            ...
            <main class="md:max-w-screen-xl mx-auto">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <BlogCard v-for="article in posts_data['posts']" :key="article.id" :article="article" />
                </div>
...
const fetchData = async () => {
    ...
    const { data: articles} = await useFetchApi(`wp/v1/latest-posts?page_size=${page_size}&page_no=${page_no.value}&cat_slug=${route.params.slug}`);
